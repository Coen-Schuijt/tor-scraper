...
def append_to_online(URL,online_list):
    online_list.append(URL)
    return

async def check_online(URL,online_list):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(URL) as response:
                ...
                if response == 200:
                    ...
                    append_to_online(URL,online_list)
    except:
...
def main():
    ...
    tasks = []
    for URL in URLS:
        task = asyncio.ensure_future(check_online(URL,online_list))
        tasks.append(task)
